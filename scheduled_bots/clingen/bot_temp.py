...
df.columns = ['Gene', 'HGNC Gene ID', 'Disease', 'MONDO Disease ID', 'SOP', 'Classification', 'Report Reference URL',
              'Report Date']
...
df[
    'Status'] = "pending"  # "Status" column with 'pending' for all cells: 'error' or 'complete' (meaning previously logged within 180 days)
...
for index, row in df.iterrows():
    ...
    if MONDO_qlength == 1:
        MONDO_qid = result_MONDO["results"]["bindings"][0]["disease"]["value"].replace(
