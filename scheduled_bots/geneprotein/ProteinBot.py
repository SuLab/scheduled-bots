...
class Protein:
    ...
    def __init__(self, record, organism_info, gene_wdid, login, write=True):
        """
    ...
    def parse_external_ids(self):
        ...
        ############
        if 'protein' in self.record['ensembl']['@value']:
            # Ensembl Protein ID
            external_ids['Ensembl Protein ID'] = self.record['ensembl']['@value']['protein']

        if 'protein' in self.record['refseq']['@value']:
            # RefSeq Protein ID
    ...
    def create_statements(self):
        ...
        ############
        uniprot_ref = make_ref_source(self.record['uniprot']['@source'], PROPS['UniProt ID'], self.external_ids['UniProt ID'],
                                      login=self.login)
        ...
        # found in taxon
        s.append(wdi_core.WDItemID(self.organism_info['wdid'], PROPS['found in taxon'], references=[entrez_ref]))
...
class MammalianProtein(Protein):
    """
...
class ProteinBot:
    ...
    def run(self, records, total=None, write=True):
        for record in tqdm(records, mininterval=2, total=total):
            ...
            protein = Protein(record, self.organism_info, gene_wdid, self.login, write=write)
            protein.create_item()
